...
class FileOrganizerEnv(Environment):
    def __init__(self):
        super().__init__()
        # Explicitly defined 3 task sets for the grader
        self.task_sets = [
    ...
    def reset(self, episode_id: Optional[str] = None, seed: Optional[int] = None) -> FileObservation:
        """Resets the environment state and rotates tasks."""
        # Select task and increment counter
        ...
        return FileObservation(
            ...
            last_action_status=f"Task {self.current_task_idx} Loaded",
            reward=0.01, 
            done=False
        )
    ...
    def step(self, action: FileAction) -> FileObservation:
        ...
        if is_valid:
            ...
            status = f"Rejected: {category} invalid for {file_name}"
